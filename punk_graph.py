...
class NFTGraphBuilder:
    ...
    async def get_all_punk_holders(self, punk_contract: str = "0xb47e3cd837ddf8e4c57f05d70ab865de6e193bbb") -> Set[str]:
        """Fetch all holders of CryptoPunks"""
        logger.info(f"Fetching all holders of CryptoPunks contract: {punk_contract}")
        
        all_owners = set()
        page_key = None
        pages_fetched = 0
        max_pages = 100  # CryptoPunks has ~3-4k holders, so we need more pages
        ...
        while pages_fetched < max_pages:
            ...
            try:
                logger.info(f"Fetching punk holders page {pages_fetched + 1}")
                ...
                async with self.session.get(url, params=params) as response:
                    ...
                    if response.status != 200:
                        logger.error(f"Error fetching punk holders: {response.status}")
                ...
                logger.error(f"Error fetching punk holders: {e}")
                break
        
        logger.info(f"Found total of {len(all_owners)} unique CryptoPunk holders")
        
        # Cache the punk contract owners
        self.contract_to_owners[punk_contract.lower()] = all_owners
...
async def main():
    ...
    logger.info("Starting CryptoPunks network builder with Alchemy...")
    
    async with NFTGraphBuilder(api_key, rate_limit=20, checkpoint_dir="./punk_checkpoints") as builder:
        # First, fetch all CryptoPunk holders
        logger.info("Fetching all CryptoPunk holders...")
        punk_holders = await builder.get_all_punk_holders()
        
        if not punk_holders:
            logger.error("Failed to fetch punk holders!")
            return
        
        logger.info(f"Starting graph build with {len(punk_holders)} CryptoPunk holders")
        
        # Build the graph starting with all punk holders
        graph = await builder.build_graph(
            start_wallets=list(punk_holders),  # Convert set to list
            max_depth=2,  # Reduced depth since we're starting with many wallets
            max_nodes=50000,  # Increased limit for punk community
            resume_from_checkpoint=resume_from_checkpoint
        )
        
        output_file = "cryptopunk_network.gexf"
        logger.info(f"Saving graph to {output_file}")
        nx.write_gexf(graph, output_file)
        
        # Save some statistics
        stats_file = "cryptopunk_network_stats.json"
        stats = {
            "total_punk_holders": len(punk_holders),
